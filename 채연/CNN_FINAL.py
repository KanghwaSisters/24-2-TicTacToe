...
Original file is located at
    https://colab.research.google.com/drive/1M2cVWzNyhpeNSZOkRyDh78513JwItVL-
"""
...
# 데이터 증강 이후 필요없는 파일이 생긴 경우 삭제하는 코드
import os

data_path = '/content/drive/MyDrive/kanghwa/ttt/image_black'
file_list = [f for f in os.listdir(data_path) if f.lower().endswith(('.jpg','.JPG'))]

# '_blurred_fog'가 포함된 파일 삭제
for file_name in file_list:
    if '_elastic' in file_name:
        file_path = os.path.join(data_path, file_name)
        os.remove(file_path)  # 파일 삭제
        print(f"Deleted: {file_name}")

print("Deletion of '_blurred_fog' files completed!")
...
dataset = TicTacToeCellDataset(image_dir='/content/drive/MyDrive/kanghwa/ttt/image_black', label_dir='/content/drive/MyDrive/kanghwa/ttt/labels', transform=transform)
...
# 이미지가 None인지 확인 (파일 읽기 실패 방지)
if img is None:
    print("Failed to load image. Check the file path.")
else:
    # 가우시안 블러 적용
    blurred_img = cv2.GaussianBlur(img, (47,47), 0)  # (11, 11): 커널 크기, 0: 자동 시그마 계산
...
# Random Fog 효과 추가 함수
def add_random_fog(img, fog_intensity=0.4, blur_strength=15):
    height, width, _ = img.shape
    mask = np.random.rand(height, width) < fog_intensity
    # 흐림 효과 적용
    foggy_img = cv2.GaussianBlur(img, (blur_strength, blur_strength), 0)
...
if img is None:
    ...
    # 가우시안 블러 적용
    masked_img = add_random_fog(img, fog_intensity=0.9, blur_strength=55)
...
for label_file in tqdm(label_files, desc="Copying label files"):
    ...
    # 복사본 라벨 경로 (_fog 추가)
    base_name, ext = os.path.splitext(label_file)
    fog_label_path = os.path.join(data_path, f"{base_name}_affine{ext}")
...
# 이미지 경로
img_path = '/content/drive/MyDrive/kanghwa/ttt/image_black/35.jpg'

# 이미지 읽기
img = cv2.imread(img_path)
elastic_jitter = transforms.ElasticTransform(alpha=80.0,
                                           sigma=2.0,
                                           fill=0)
if img is None:
    print("Failed to load image. Check the file path.")
    ...
    img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
    pil_img = Image.fromarray(img)
    elastic_img = elastic_jitter(pil_img)
    elastic_img = cv2.cvtColor(np.array(elastic_img), cv2.COLOR_RGB2BGR)

    print("Original Image:")
    cv2_imshow(img)

    print("Elastic Image:")
    cv2_imshow(elastic_img)
...
elastic_jitter = transforms.ElasticTransform(alpha=80.0,
                                           ...
                                           fill=0)
# Randomfog augmentation 적용
for file_name in tqdm(file_list, desc="Applying elastic augmentation"):
    # 원본 이미지 경로
    img_path = os.path.join(data_path, file_name)
    ...
    base_name, ext = os.path.splitext(file_name)
    elastic_output = os.path.join(data_path, f"{base_name}_elastic{ext}")
    cv2.imwrite(elastic_output, elastic_img)
...
for label_file in tqdm(label_files, desc="Copying label files"):
    ...
    # 복사본 라벨 경로 (_fog 추가)
    base_name, ext = os.path.splitext(label_file)
    fog_label_path = os.path.join(data_path, f"{base_name}_elastic{ext}")
